...
class ListaUnica:
    ...
    def verifica_tipo(self, elem):
        if type(elem) != self.elem_class:
            raise TypeError('Tipo Invalidao')
...
class DadoAgenda:
    ...
    def nome(self, valor):
        if type(valor)!=Nome:
            raise ValueError("nome deve ser um instância da classe Nome")
        self.__nome = valor
...
class AppAgenda:
    ...
    def menu_telefones(self, dados):
        while True:
            ...
            if operacao == 'd' and len(dados.telefones) > 0:
    ...
    def grava(self):
        if self.ultimo_nome != None:
            print("Ultimo nome utilizado foi %s" % self.ultimo_nome)
